...
def _world() -> WorldState:
    return WorldState(
        ...
        actors={
            ...
            "B": ActorState(actor_id="B", name="Event-time witness", scene_id="S1", zone_id="Z1", capabilities={"SPEAK"}),
            "C": ActorState(actor_id="C", name="Historical nonwitness", scene_id="S1", zone_id="Z1", capabilities={"SPEAK"}),
        },
        objects={
            "DOOR": ObjectState(object_id="DOOR", name="木门", scene_id="S1", zone_id="Z1", fragility=0.25)
        },
        npc_minds={"B": NPCMindState(npc_id="B", role="WITNESS"), "C": NPCMindState(npc_id="C", role="NEWCOMER")},
        scenes={
            "S1": SceneState(
                scene_id="S1",
...
def test_historical_nonwitness_then_later_visibility_cannot_retroactively_mint_witness_or_cause():
    ...
    historical = derive_world_echo_opportunity(world=world, speaker_npc_id="C", entity_id="DOOR", source_event_id=source.event_id)
    ...
    after_visibility_echo = derive_world_echo_opportunity(world=later, speaker_npc_id="C", entity_id="DOOR", source_event_id=source.event_id)
    assert after_visibility_echo == historical
    assert after_visibility_echo.opportunity is None
    gap = assess_current_visual_observation_gap(world=later, observer_actor_id="C", entity_id="DOOR")
    ...
    with pytest.raises(CurrentObservationEvidenceError, match="NO_TRUSTED_OBSERVATION_TRIGGER"):
        capture_current_visual_observation(world=later, observer_actor_id="C", entity_id="DOOR")
...
def test_later_visibility_cannot_upgrade_simulator_known_actor_into_culprit_attribution():
    ...
    assert source.actor_id == "A"
    later = _LaterCurrentVisibilityView(world, entity_id="DOOR", observer_actor_id="C")
    decision = derive_world_echo_opportunity(world=later, speaker_npc_id="C", entity_id="DOOR", source_event_id=source.event_id)
...
def test_restart_replay_preserves_historical_nonwitness_then_later_visibility_disposition():
    ...
    first_echo = derive_world_echo_opportunity(world=first_later, speaker_npc_id="C", entity_id="DOOR", source_event_id=source.event_id)
    first_gap = assess_current_visual_observation_gap(world=first_later, observer_actor_id="C", entity_id="DOOR")
    ...
    second_echo = derive_world_echo_opportunity(world=second_later, speaker_npc_id="C", entity_id="DOOR", source_event_id=replayed_source.event_id)
    second_gap = assess_current_visual_observation_gap(world=second_later, observer_actor_id="C", entity_id="DOOR")
