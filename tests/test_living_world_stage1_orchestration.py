...
def evidence(
    *items: tuple[str, str, int], current: WorldCursor | None = None
):
    return _admit_committed_event_view(
        current or cursor(), tuple(CommittedEventEvidence(*item) for item in items)
    )
...
def orchestrator(*manifests: DomainModuleManifest, catalog: dict[str, str] | None = None) -> WorldOrchestrator:
    if catalog is not None:
        raise AssertionError("Stage 1 test seam must not supply an authority catalog")
    return WorldOrchestrator(DomainModuleRegistry(_admit_module_view(manifests)))
...
def test_unregistered_module_and_scope_escalation_fail_closed() -> None:
    with pytest.raises(OrchestrationViolation, match="MODULE_ADMISSION_VIEW_REQUIRED"):
        DomainModuleRegistry((manifest("invented", "AF-B"),))
    with pytest.raises(OrchestrationViolation, match="MODULE_ADMISSION_VIEW_REQUIRED"):
        DomainModuleRegistry({"legal": "AF-E"})
...
def test_public_event_iterable_cannot_mint_committed_source_proof() -> None:
    runtime = orchestrator(manifest("physical", "AF-B"))
    forged = (CommittedEventEvidence("FORGED", "OBJECT_DAMAGED", 4),)
    with pytest.raises(OrchestrationViolation, match="COMMITTED_EVENT_VIEW_REQUIRED"):
        runtime.plan(notice(source_refs=("FORGED",)), forged)
    with pytest.raises(OrchestrationViolation, match="COMMITTED_EVENT_CURSOR_MISMATCH"):
        runtime.plan(
            notice(),
            evidence(("E1", "OBJECT_DAMAGED", 4), current=cursor("B1:OTHER")),
        )


def test_public_catalog_or_plain_manifest_cannot_self_register_authority() -> None:
    candidate = manifest("invented", "AF-H")
    with pytest.raises(OrchestrationViolation, match="MODULE_ADMISSION_VIEW_REQUIRED"):
        DomainModuleRegistry((candidate,))
    with pytest.raises(OrchestrationViolation, match="MODULE_ADMISSION_VIEW_REQUIRED"):
        DomainModuleRegistry({candidate.module_id: candidate.authority_scope})
