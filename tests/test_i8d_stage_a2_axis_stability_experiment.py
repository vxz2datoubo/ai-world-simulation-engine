...
def comparison(kind: str, label: str | None = None) -> StageA2ComparisonFixture:
    return StageA2ComparisonFixture(
        comparison_id=label or f"A2:{kind}",
        comparison_kind=kind,
    )


def stage_a_fixture(**overrides) -> BranchEvidenceExperimentFixture:
    values = {
...
def make_i5_pair():
    ...
    def plausibility(route_available: bool) -> ShadowPlausibilityFixture:
    ...
    def export(route_available: bool) -> bytes:
        return export_information_opportunity_shadow_package(
            ...
            fixture=plausibility(route_available),
        )
...
def make_i7_pair(*, mutate_history_between: bool = False):
    ...
    resolution = SimulationEngine().resolve_and_commit(action, world)
    source = next(e for e in resolution.events if e.event_type == "OBJECT_DAMAGED")
    ready_filter = ReferencePrivateEchoFilter(fixture_id="A2-I7-ECHO")
...
def make_i8_pair():
    ...
    speech_action = ActionCompiler().compile(
        f"告诉店主 PROMISE_REPAIR_OBJECT:{door}",
        player,
        world,
        principal,
    )
    ...
    # A later canonical event is required by the accepted I8B callback semantics.
    # Both Storylet variants are exported only after this shared history exists,
    # so the eligible/invalid pair has the exact same I1 replay package.
    later = ActionCompiler().compile("砸货箱", player, world, principal)
    SimulationEngine().resolve_and_commit(later, world)
...
def test_core_shape_is_present_across_i5_i7_i8(pair):
    ...
    result = evaluate_stage_a2_axis_stability(
        ...
        fixture=comparison(
            ...
            f"A2-CROSS:{left_kind}:{right_kind}",
        ),
    )
    ...
    for core in (thaw_value(result.left_core_axes), thaw_value(result.right_core_axes)):
        ...
        assert core["legal_dead_end_opportunity_scarcity_risk"]["assessment"] in {
            "ABSENT",
            "RISK",
        }
...
def test_mechanism_absence_stays_not_applicable_not_bad_quality():
    ...
    assert (
        ...
        in result.right_not_applicable_mechanism_axes
    )
...
def test_false_metadata_only_claim_is_rejected_when_source_changed():
    ...
    assert (
        "AUTHORED_METADATA_COMPARISON_REQUIRES_IDENTICAL_SOURCE"
        in result.integrity_failures
    )
...
def test_status_change_comparison_rejects_different_i1_history():
    ...
    assert result.outcome == "COMPARISON_NOT_VALID"
    assert (
        "UPSTREAM_STATUS_COMPARISON_REQUIRES_SAME_I1_REPLAY"
        in result.integrity_failures
    )
...
def test_caller_cannot_mint_core_evidence():
    ...
    with pytest.raises(
        ValueError,
        match="I8D_A2_CALLER_AUTHORED_CORE_EVIDENCE_FORBIDDEN",
    ):
        evaluate_stage_a2_axis_stability(
            left_stage_a_package=package,
            right_stage_a_package=package,
...
def test_unsupported_comparison_kind_fails_closed():
    ...
    package = stage_a("I5A_INFORMATION_OPPORTUNITY", source)
    forged = StageA2ComparisonFixture(
        comparison_id="A2-BAD-KIND",
        comparison_kind="MAXIMIZE_ENGAGEMENT",
    )
...
def test_nested_upstream_tamper_cannot_be_laundered_by_recomputed_stage_a_digest():
    ...
    envelope["payload"]["source_package_b64"] = base64.b64encode(forged_nested).decode(
        "ascii"
    )
...
def test_export_binds_exact_stage_a_hashes():
    ...
    right = stage_a(
        "I7A_WORLD_ECHO",
        source,
        recoverable_thread_refs=("THREAD:A2-HASH",),
    )
...
def test_stage_a2_outer_tamper_fails_closed():
    ...
    envelope["payload"]["expected_observation"]["outcome"] = (
...
def test_recomputed_a2_outer_still_cannot_launder_nested_stage_a_tamper():
    ...
    envelope["payload"]["left_stage_a_b64"] = base64.b64encode(tampered_left).decode(
        "ascii"
    )
    envelope["payload"]["left_stage_a_sha256"] = hashlib.sha256(
        tampered_left
    ).hexdigest()
...
def test_recomputed_outer_cannot_forge_expected_observation():
    ...
    with pytest.raises(
        ValueError,
        match="I8D_A2_REPLAY_OBSERVATION_MATERIALIZATION_MISMATCH",
    ):
        replay_stage_a2_axis_stability_package(refresh_outer_digest(envelope))
