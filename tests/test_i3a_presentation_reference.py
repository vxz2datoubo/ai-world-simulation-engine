...
from runtime.awrse.presentation_reference import (
...
EVENTS = [
    ...
    {
        "event_id": "E-I3A-002-WEAR-BOOTS",
        "cursor": 102,
        "actor_id": ACTOR,
        "kind": "WEAR_SLOT",
        "slot": "feet",
        "object_ref": "OBJ-BOOTS",
    },
    {
        "event_id": "E-I3A-003-REMOVE-BOOTS",
        "cursor": 103,
        "actor_id": ACTOR,
        "kind": "CLEAR_SLOT",
        "slot": "feet",
    ...
    {
        "event_id": "E-I3A-004-DRESS-RIGHT-FOREARM",
        "cursor": 104,
        "actor_id": ACTOR,
        "kind": "APPLY_DRESSING",
        "dressing_id": "DRESS-RF-1",
        "body_region": "FOREARM",
        "side": "RIGHT",
    ...
    {
        "event_id": "E-I3A-005-MUD-COAT",
        "cursor": 105,
        "actor_id": ACTOR,
        "kind": "SET_SURFACE",
        "surface_state_id": "SURF-COAT-MUD",
        "target_ref": "OBJ-COAT",
        "surface_type": "MUD",
        "intensity": 0.4,
    },
        ...
        "dressings": [
            {
                ...
                "appearance_state": thaw_value(item["appearance_state"]),
            }
            for item in reference.dressing_states
        ],
        "surface_states": [
            {
                "surface_state_id": item["surface_state_id"],
...
def test_renderer_body_region_drift_fails_closed():
    ...
    result = validate_mock_render_claims(reference, claims)
    assert any("body_region" in item for item in result.contradictions)
...
def test_removed_dressing_cannot_be_resurrected_by_old_generated_claim():
    events = copy.deepcopy(EVENTS)
    events.append({
        "event_id": "E-I3A-006-REMOVE-DRESSING",
        "cursor": 106,
        "actor_id": ACTOR,
        "kind": "REMOVE_DRESSING",
        "dressing_id": "DRESS-RF-1",
    })
    ...
    result = validate_mock_render_claims(reference, claims)
    assert "OBSOLETE_OR_UNKNOWN_DRESSING:DRESS-RF-1" in result.contradictions
...
def test_renderer_or_generated_pixels_cannot_request_upstream_mutation():
    ...
    result = validate_mock_render_claims(reference, claims)
    assert set(result.unauthorized_claims) == {
...
def test_locator_migration_does_not_change_presentation_identity_or_history():
    ...
    migrated["MAT-LINEN"]["locator_id"] = "LOC-LINEN-B"
...
def test_view_change_does_not_change_actor_presentation_truth():
    west = build(view_id=VIEWS[0])
    east = build(view_id=VIEWS[1])
...
def test_replay_package_is_byte_deterministic_and_rebuilds_exact_reference():
    ...
    kwargs = dict(
        ...
        asset_registry=copy.deepcopy(ASSETS),
        valid_view_ids=VIEWS,
    )
    package_a = export_replay_package(**kwargs)
    package_b = export_replay_package(**kwargs)
    assert package_a == package_b
    rebuilt = replay_package(package_a)
    assert rebuilt == reference


def test_replay_payload_tampering_is_rejected_before_rebuild():
    reference = build()
    package = export_replay_package(
        reference=reference,
        events=EVENTS,
        inventory_object_refs=INVENTORY,
...
def test_dressing_appearance_cannot_mint_functional_injury():
    ...
    result = validate_mock_render_claims(reference, claims)
    assert "DRESSING_APPEARANCE_CANNOT_AUTHOR_FUNCTIONAL_INJURY" in result.unauthorized_claims
...
def test_cleared_surface_cannot_be_resurrected():
    ...
    result = validate_mock_render_claims(reference, claims)
    assert "OBSOLETE_OR_UNKNOWN_SURFACE:SURF-COAT-MUD" in result.contradictions
