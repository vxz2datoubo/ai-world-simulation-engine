...
def make_world() -> WorldState:
    return WorldState(
        ...
        actors={
            # Same display name is intentional: machine identity must not merge.
        ...
        scenes={
            "SCENE_001": SceneState(
                "SCENE_001", ["asset://i4a/scene"], [DOOR, HIDDEN_BOX], [PLAYER_A, PLAYER_B, NPC]
            )
        },
        principal_actor_bindings={PRINCIPAL_A: {PLAYER_A}, PRINCIPAL_B: {PLAYER_B}},
        reachable_pairs={(PLAYER_A, DOOR), (PLAYER_A, HIDDEN_BOX), (PLAYER_B, DOOR), (PLAYER_B, HIDDEN_BOX)},
...
def build_source_world():
    ...
    damage_event = next(event for event in witnessed.events if event.event_type == "OBJECT_DAMAGED")

    # Strict marker is still UNTRUSTED speech. It says something about an exact
    # event; it cannot alter that event's canonical actor_id.
...
def test_i4a_golden_one_npc_two_players_preserves_witness_heard_hidden_and_sparse_relationship():
    ...
    reference = build_npc_memory_reference(world=world, npc_id=NPC, player_ids=[PLAYER_A, PLAYER_B])
    ...
    memory_sources = {
        ref for item in reference.episodic_memories for ref in item["source_world_event_refs"]
    }
...
def test_i4a_world_truth_without_acquisition_path_does_not_become_memory():
    ...
    reference = build_npc_memory_reference(world=world, npc_id=NPC, player_ids=[PLAYER_A, PLAYER_B])
    assert hidden_event_id in reference.context_bundle["forbidden_hidden_fact_refs"]
    assert all(hidden_event_id not in item["source_world_event_refs"] for item in reference.episodic_memories)
...
def test_i4a_caller_or_summary_mapping_cannot_mint_memory_evidence():
    ...
    with pytest.raises(ValueError, match="I4A_CALLER_AUTHORED_MEMORY_EVIDENCE_FORBIDDEN"):
        build_npc_memory_reference(
            ...
            caller_memory_evidence={"memory_id": "MEMORY:FORGED", "claim": "I remember everything"},
        )


def test_i4a_world_echo_eval_fixture_cannot_be_supplied_as_memory_authority():
    _, world, _, _ = build_source_world()
    ...
    with pytest.raises(ValueError, match="I4A_CALLER_AUTHORED_MEMORY_EVIDENCE_FORBIDDEN"):
        build_npc_memory_reference(
            world=world, npc_id=NPC, player_ids=[PLAYER_A, PLAYER_B], caller_memory_evidence=evidence
        )
...
def test_i4a_cross_npc_acquisition_ref_fails_closed():
    world = make_world()
    speech = Event(
        "E-SOURCE", "SPEECH_UTTERED", PLAYER_A, "SCENE_001", BASELINE_VERSION,
        {"literal_content": "hello", "trust_class": "UNTRUSTED_DATA", "authority": "NONE_OVER_TARGET_INTERNAL_STATE"},
    )
    wrong = Event(
        "E-WRONG-NPC", "NPC_KNOWLEDGE_ACQUIRED", PLAYER_A, "SCENE_001", BASELINE_VERSION,
        {"npc_id": "NPC_OTHER", "mode": "HEARD", "source_event_id": speech.event_id, "speaker_id": PLAYER_A},
    )
    world.event_log = [speech, wrong]
    world.committed_event_ids = {speech.event_id, wrong.event_id}
    world.state_version = 2
    world.npc_minds[NPC].memories = [wrong.event_id]
    world.npc_minds[NPC].knowledge_boundary_refs = [speech.event_id]
    with pytest.raises(ValueError, match="I4A_CROSS_NPC_MEMORY_LEAK"):
        build_npc_memory_reference(world=world, npc_id=NPC, player_ids=[PLAYER_A, PLAYER_B])


def test_i4a_relationship_event_does_not_itself_mint_memory():
    _, world, _, _ = build_source_world()
    relationship_events = {event.event_id for event in world.event_log if event.event_type == "RELATIONSHIP_CHANGED"}
    reference = build_npc_memory_reference(world=world, npc_id=NPC, player_ids=[PLAYER_A, PLAYER_B])
    memory_sources = {ref for item in reference.episodic_memories for ref in item["source_world_event_refs"]}
...
def test_i4a_replay_package_is_byte_deterministic():
    baseline, world, _, _ = build_source_world()
    first = export_npc_memory_replay_package(baseline=baseline, world=world, npc_id=NPC, player_ids=[PLAYER_A, PLAYER_B])
    second = export_npc_memory_replay_package(baseline=baseline, world=world, npc_id=NPC, player_ids=[PLAYER_A, PLAYER_B])
    assert first == second


def test_i4a_outer_replay_tamper_fails_before_rebuild():
    baseline, world, _, _ = build_source_world()
    envelope = json.loads(export_npc_memory_replay_package(
        baseline=baseline, world=world, npc_id=NPC, player_ids=[PLAYER_A, PLAYER_B]
    ).decode("utf-8"))
...
def test_i4a_inner_i1_history_tamper_survives_no_outer_digest_laundering():
    baseline, world, _, _ = build_source_world()
    envelope = json.loads(export_npc_memory_replay_package(
        baseline=baseline, world=world, npc_id=NPC, player_ids=[PLAYER_A, PLAYER_B]
    ).decode("utf-8"))
