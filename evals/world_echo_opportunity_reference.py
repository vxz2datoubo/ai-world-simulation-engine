...
def _exact_committed_event(world: WorldState, event_id: str) -> tuple[int, Event]:
    matches = [(index, event) for index, event in enumerate(world.event_log, start=1) if event.event_id == event_id]
    if not matches:
...
def _event_position(world: WorldState, event: Event) -> int:
    return next(index for index, candidate in enumerate(world.event_log, start=1) if candidate.event_id == event.event_id)
...
def derive_world_echo_opportunity(*, world: WorldState, speaker_npc_id: str, entity_id: str, source_event_id: str) -> WorldEchoDecision:
    ...
    if not acquisitions:
        return WorldEchoDecision(status="NO_VALID_OPPORTUNITY", reason="NO_PROVEN_ACQUISITION_OR_CURRENT_PERCEPTION")
    visual_object_acquisitions = [event for event in acquisitions if str(event.payload.get("mode", "")) == "SAW"]
    if not visual_object_acquisitions:
        return WorldEchoDecision(status="NO_VALID_OPPORTUNITY", reason="ACQUISITION_MODE_NOT_SUPPORTED_BY_V2_OBJECT_ECHO_POLICY")
    ...
    return WorldEchoDecision(status="CANDIDATE_BLOCKED_PENDING_CURRENT_PERCEPTION", reason="OBJECT_DAMAGE_ACQUIRED_CAUSAL_ACTOR_NOT_PROVEN", opportunity=opportunity)
