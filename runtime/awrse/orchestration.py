...
_REQUIRED_STAGE1_PROVENANCE = Stage1Provenance(
    task_id="LW-STAGE1-ORCH-001-R2",
    snapshot_id="AWRSE-LW-STAGE1-ORCH-001-R2-SNAPSHOT-001",
    baseline_sha="60391e53c805efb31a5bafce7d08564a97f8feec",
)
...
class _CommittedEventView:
    ...
    _cursor: WorldCursor
    _authority_root: object

    def __post_init__(self) -> None:
        validator = getattr(self._authority_root, "validate_event_view", None)
        if not callable(validator) or not validator(self._cursor, self._events):
...
def _admit_committed_event_view(*_args: object, **_kwargs: object) -> _CommittedEventView:
    """Removed R1 seam: caller material can never become committed proof."""
    raise OrchestrationViolation("AUTHORITY_ROOT_REQUIRED")
...
class _ModuleAdmissionView:
    ...
    def __post_init__(self) -> None:
        validator = getattr(self._authority_root, "validate_module_view", None)
        if not callable(validator) or not validator(self._manifests, self._admission_digest):
            raise OrchestrationViolation("MODULE_ADMISSION_VIEW_UNTRUSTED")


def _admit_module_view(*_args: object, **_kwargs: object) -> _ModuleAdmissionView:
    """Removed R1 seam: manifests are admitted only by the governed registry."""
    raise OrchestrationViolation("AUTHORITY_ROOT_REQUIRED")
